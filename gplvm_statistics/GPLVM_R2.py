...
class GPLVM_r2(ReducedStatistic):
    """
    Calculate the fit of the GPLVM for a given dataset.
    (MAP Version)
    """
    ...
    @property
    def name(self) -> str:
        return "GPLVM-Fit-Custom-MLE"

    @property
    def identifier(self) -> str:
        return "gplvm_fit_custom_numpy_mle"

    @property
    def labels(self) -> list[str]:
        return ["manifold", "gplvm", "numpy_backend", "mle"]

    def _shuffle_rows_per_column(self, X: np.ndarray, seed: Optional[Union[int, np.random.Generator]] = None, copy: bool = True) -> np.ndarray:
        """
    ...
    def _log_likelihood(self, params, Y, q, n, d, return_grad=True):
        """
        Calculate Objective function (MAP estimation).
        Minimizing: 
        """
        # 1. unpack parameters
        X_flat = params[:-3]
        ...
        # Negative Log Likelihood plus log prior
    ...
    def compute(self, data: np.ndarray) -> np.ndarray: 
        """
        Fit GPLVM and return the final score.
        """
        # pre
        ...
        # initialisation
        # PCA initialisation
        u, s, vt = svd(Y_centered, full_matrices=False)
        X_init = u[:, :q] * s[:q]
        ...
        X_optimized = optimized_params[:-3].reshape(n, q)
        hyperparams_optimized = optimized_params[-3:] 

        # 4. Compute Score

        nll_real = self._log_likelihood(optimized_params, Y_centered, q, n, d_out, return_grad=False)
        
        if np.isinf(nll_real):
            print("DEBUG: Real NLL is INF. Returning 0.0")
            return np.array([0.0])

        log_predictive_density = -nll_real

        # Null Model
        null_lpds = []
        for _ in range(5):
            X_shuffled = self._shuffle_rows_per_column(X_optimized)
            params_null = np.concatenate([X_shuffled.flatten(), hyperparams_optimized])
            nll_null = self._log_likelihood(params_null, Y_centered, q, n, d_out, return_grad=False)
            null_lpds.append(-nll_null)
        
        null_log_predictive_density = np.mean(null_lpds)

        
        # difference
        raw_diff = log_predictive_density - null_log_predictive_density
        
        
        
        
        # Score
        score = raw_diff / np.abs(null_log_predictive_density)
